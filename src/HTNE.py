...
class CTDNE_data(temporal_data):
    ...
    def __init__(self,  edge_list=None,src_path=None, vocal=True):
        ...
        self.nodes = list(pd.concat([self.df_data['src'], self.df_data['tar']]).unique())
        # By default sorted
        self.vocal = vocal
    ...
    def _find_neighbors(self):
        ...
        temp_nbrs = defaultdict(list, {node: [] for node in range(self.num_nodes)})
...
def find_hist(df_data):
    ...
    end_time = time.time()
    print("Elapsed time:", end_time - start_time, "seconds for finidng node history")
    return node2hist
...
class HTNE_dataset(CTDNE_data):
    def __init__(self, edge_list=None, src_path=None, neg_size=10, hist_len=2, vocal=True):
        ...
        all_nodes = pd.concat([self.df_data['src'], self.df_data['tar']])
        self.degrees = all_nodes.value_counts().to_dict()
        ...
        #         idx += 1
        self.idx2src_id = np.array([src for src, targets in self.node2hist.items() for _ in targets])
        self.idx2tar_id = np.array([tar_id for targets in self.node2hist.values() for tar_id in range(len(targets))])
    ...
    def calculate_degrees(self):
        # Concatenate 'src' and 'tar' columns and count occurrences
        all_nodes = pd.concat([self.df_data['src'], self.df_data['tar']])
        all_degrees = all_nodes.value_counts().to_dict()

        # Convert keys to integers
        self.degrees = {int(k): v for k, v in all_degrees.items()}
        print(self.degrees[0])
    ...
    def __getitem__(self, idx):
        ...
        # print(self.node2hist[idx])
        # raise
        # tar, dat, lab = self.node2hist[src][tar_idx]
        # src = idx
        # tar, dat, lab = random.choice(self.node2hist[idx])
        # tar_idx = tar
        src = idx
        tar_idx = random.choice(range(len(self.node2hist[idx])))
        tar, dat, lab = self.node2hist[idx][tar_idx]
...
# Your NodeWiseSampler definition here
class NodeWiseSampler(Sampler):
    def __init__(self, src_to_indices, batch_size):
        self.src_to_indices = src_to_indices
        self.batch_size = batch_size

    def __iter__(self):
        all_nodes = list(self.src_to_indices.keys())
        random.shuffle(all_nodes)
        
        for i in range(0, len(all_nodes), self.batch_size):
            yield all_nodes[i:i+self.batch_size]

def custom_collate(batch, src_to_indices):
    batch_indices = []
    for src_nodes in batch:
        for src in src_nodes:
            indices = src_to_indices.get(src, [])
